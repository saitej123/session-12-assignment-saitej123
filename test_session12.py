import session12_part_1
import session12_part_2
from session12_part_1 import *
from session12_part_2 import *
...
def test_indentations_1():
    ...
    lines = inspect.getsource(session12_part_1)
    spaces = re.findall('\n +.', lines)
    for space in spaces:
...
def test_indentations_2():
    ''' Returns pass if used four spaces for each level of syntactically significant indenting.'''
    lines = inspect.getsource(session12_part_2)
    spaces = re.findall('\n +.', lines)
    for space in spaces:
        assert len(space) % 4 == 2, "Your script contains misplaced indentations"
        assert len(re.sub(
...
def test_Polygon_lazy_prop():
    p1 = Polygon(10, 12)
    sq_iter = iter(p1)
    assert bool(next(sq_iter))
